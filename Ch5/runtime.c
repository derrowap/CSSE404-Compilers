...
void cheney(int64_t** rootstack_ptr) {
  ...
  // copy root stack pointers to tospace
  for (int64_t** root_ptr = rootstack_begin; root_ptr != rootstack_ptr;
        ++root_ptr) {
    copy_vector(root_ptr);
  }
  ...
  while (scan_ptr != free_ptr) {
    ...
    // bits tell if location is another pointer or not
    int64_t is_ptr_bits = get_ptr_bitfield(tag);
    // iterate through vector
    scan_ptr++;
    while (scan_ptr != next_ptr) {
      if ((is_ptr_bits & 1)) {
        // scan_ptr is really a pointer to a pointer
  ...
  // swap to space and from space
  int64_t* tmp = tospace_begin;
...
void copy_vector(int64_t** vector_ptr_loc) {
  ...
  if (is_forwarding(tag)) {
    // already copied vector only needs pointer update
